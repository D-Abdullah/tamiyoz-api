...
Class Upload {
     ...
     * @return  void
     */
    public function __construct($allowext, $thumbsConfig, $maxsize, $uploadfolder, $thumbsimages) {
        $this->_allowext = $allowext;
        $this->_thumbsConfig = $thumbsConfig;
        $this->_maxsize = $maxsize;
        $this->_error = array();
        $this->_fileinfo = array();
        $this->_uploadfolder = $uploadfolder;
//        $this->_logopath = $_SERVER['DOCUMENT_ROOT'] . "/" ."link/api/uploads/logo.png";
        $this->_thumbsimages = $thumbsimages;
    }

    // public static function thumbsUpload($allowext, $thumbsConfig, $maxsize, $uploadfolder, $thumbsimages) {
    //     $instance = new self();
    //     $instance->_allowext = $allowext;
    //     $instance->_thumbsConfig = $thumbsConfig;
    //     $instance->_maxsize = $maxsize;
    //     $instance->_error = array();
    //     $instance->_fileinfo = array();
    //     $instance->_uploadfolder = $uploadfolder;
    //     $instance->_thumbsimages = $thumbsimages;
    //     return $instance;
    // }
     ...
     */
    public function Upload_File($filearray, $maxsize = NULL, $imgName = NULL) { 
        ...
        if (is_array($filearray) OR !is_null($filearray[type]) OR
            ...
            $filearray[place] = $_SERVER['DOCUMENT_ROOT'] . "/" . $this->_uploadfolder . "/";
            $filearray[nplace] = $_SERVER['DOCUMENT_ROOT'] . "/" . $this->_uploadfolder . "/" . $filearray[newname];
            // get file keywords
            ...
            if ($this->is_img($filearray[ext]) and $this->_thumbsimages == true) { 
                ...
                foreach ($this->_thumbsConfig as $key => $value) {
                    $filearray[thplace][$key] = $_SERVER['DOCUMENT_ROOT'] . "/" . $this->_uploadfolder . "/thumbs/" . $value['name'] . '_' . $filearray[newname];
                }
            }
                ...
                if ($up) {
                    ...
                    if ($this->_thumbsimages == true) {
                        ...
                        foreach ($this->_thumbsConfig as $key => $value) {
                            ...
                            // create file thumbs
                            $thumbs = $this->createthumb($this->_fileinfo[place] . $this->_fileinfo[newname], $this->_fileinfo[ext], $this->_fileinfo[thplace][$key], $value['width'], $value['hight']);
                            ...
                            if (!$thumbs) {
                                ...
                                @unlink($this->_fileinfo[thplace][$key]);
