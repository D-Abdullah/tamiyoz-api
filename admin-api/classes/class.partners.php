...
function getSomePartners($aStart, $aLimit, $sort, $type, $searchName) {
...
function getSearchPartnersCount($sort, $type, $searchName) {
...
function getPartnersCount() {
	$sql = "SELECT COUNT(`id`) as 'count' FROM `partners` ";
	return $this -> mDb -> getOne($sql); 
}

function addEditPartner($request, $img, $partner_id) {
    ...
    if ($partner_id === '' && $request['operation'] === 'add')
    {
        // Add
...
//         echo $sql; die();
        $this -> mDb ->query($sql);
        $last_partner_id = $this -> mDb ->getLastInsertId();
        foreach ($newDataLangObj as $key => $value) {
            $stmt = "INSERT INTO `partners_langs`(`partner_id`, `lang_code`, `name`) ";
            $stmt .=" VALUES ('{$last_partner_id}', '{$value['lang_code']}', '{$value['country_name']}')";
    ...
    else
    {
        ...
        $check_query = "SELECT `id` FROM `partners` WHERE `id` = '{$partner_id}'";
        ...
        if ($check_result === false) {
            ...
            if ($img != '' || $img != null) {
                ...
                $img_query = "SELECT `img` FROM `partners` WHERE `id` = '{$partner_id}'";
                $img_result = $this -> mDb -> getOne($img_query);

                $old_img = $_SERVER['DOCUMENT_ROOT'].'/' . $this->mConfig['uploads_path'] . 'partners/' . $img_result;
...
//             echo $sql; die();
            ...
            foreach ($newDataLangObj as $key => $value) {
                ...
                $stmt = "UPDATE `partners_langs` SET";
                $stmt .=" `name`='{$value['country_name']}' ";
               
                $stmt .=" WHERE `lang_code`='{$value['lang_code']}' AND `partner_id` = '{$partner_id}'";
...
function getOnePartner($id) {
    ...
    $sql = "SELECT `id`, `added_by`, `status`, `img` FROM `partners`";
    $sql .= " WHERE `id` = '{$id}'";
    $result = $this -> mDb -> getRow($sql); 

    $stmt = "SELECT cl.`lang_code`, cl.`name` FROM `partners_langs` cl";
    $stmt .= " WHERE cl.`partner_id` = '{$id}'";
...
function deletePartner($ids) {
    ...
    $sqll = "DELETE FROM `partners_langs` ";
    $sqll.= " WHERE `partner_id` IN ".$tempids;

    $this-> mDb-> query($sqll);

    $catsql = "DELETE FROM `partners` ";
    ...
    if($this-> mDb-> query($catsql)){
        ...
        if(count($images)>0){
            for ($i=0;$i<count($images);$i++){
                $old_img = $_SERVER['DOCUMENT_ROOT'] . '/' . $this->mConfig['uploads_path'] . 'partners/'. $images[$i]['img'];
                if (file_exists($old_img)) {
                    @unlink($old_img);
                }
            }
