...
if (isset($token) && isset($verify) && $verify) {
    ...
    if ($_GET['action'] == 'getSomeSliders') {
        ...
        $categoryData = $slidersObj->getSomeSliders($start, $aItemsPerPage, $sort, $type, $searchName);

        $Data = $categoryData ? $categoryData : '';
    }

    if ($_GET['action'] == 'getSearchSlidersCount') {

        $sort = $_GET['sort'] ? $_GET['sort'] : 'id';
        $type = $_GET['type'] ? $_GET['type'] : 'DESC';

        $searchName = $_GET['searchName'] ? $_GET['searchName'] : '';
        $categorySearchData = $slidersObj->getSearchSlidersCount($sort, $type, $searchName);

        $Data = $categorySearchData ? $categorySearchData : '';
    }

    if ($_GET['action'] == 'getSlidersCount') {

        $categoryData = $slidersObj->getSlidersCount();
        $Data      = $categoryData ? $categoryData : '';
    }

    if ($_GET['action'] == 'addEditSlider') {
        ...
        $uploadFolder = $Config['uploads_path'] . 'sliders';
        ...
        $categoryData = $slidersObj->addEditSlider($_REQUEST, $temp['file'], $category_id);
        $Data = $categoryData?$categoryData:''; 
    }
...
if (isset($postdata) && !empty($postdata)) {
    ...
    if (isset($token) && isset($verify) && $verify) {
        ...
        if ($_GET['action'] == 'getOneSlider') {
            ...
            $oneCountryData = $slidersObj->getOneSlider($Req);
            $Data        = $oneCountryData ? $oneCountryData : '';
        }
 if ($_GET['action'] == 'getPartnerByParentID') {

            $oneCountryData = $slidersObj->getPartnerByParentID($Req);
            $Data        = $oneCountryData ? $oneCountryData : '';
        }

        if ($_GET['action'] == 'deleteSlider') {

            $categoryData = $slidersObj->deleteSlider($Req);
            $Data     = $categoryData ? $categoryData : '';
        }
