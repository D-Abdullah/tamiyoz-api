...
if (isset($token) && isset($verify) && $verify) {
    ...
    if ($_GET['action'] == 'getSomeCategories') {
        ...
        $categoryData = $categoryObj->getSomeCategories($start, $aItemsPerPage, $sort, $type, $searchName,$category_type);

        $Data = $categoryData ? $categoryData : '';
    }

    if ($_GET['action'] == 'getSearchCategoriesCount') {
        ...
        $categorySearchData = $categoryObj->getSearchCategoriesCount($sort, $type, $searchName,$category_type);
    ...
    if ($_GET['action'] == 'getCategoriesCount') {
        $category_type = $_GET['category_type'] ? $_GET['category_type'] : 'news';
        $categoryData = $categoryObj->getCategoriesCount($category_type);
        $Data      = $categoryData ? $categoryData : '';
    }
    ...
    if ($_GET['action'] == 'addEditCategory') {
        ...
        $categoryData = $categoryObj->addEditCategory($_REQUEST, $temp['file'], $category_id); 
        $Data = $categoryData?$categoryData:''; 
    }
...
if (isset($postdata) && !empty($postdata)) {
    ...
    if (isset($token) && isset($verify) && $verify) {
        ...
        if ($_GET['action'] == 'getOneCategory') {
            ...
            $oneCategoryData = $categoryObj->getOneCategory($Req);
            $Data        = $oneCategoryData ? $oneCategoryData : '';
        }
 if ($_GET['action'] == 'getCategoryByParentID') {

            $category_type = $_GET['category_type'] ? $_GET['category_type'] : 'news';
            $oneCategoryData = $categoryObj->getCategoryByParentID($Req,$category_type);
            $Data        = $oneCategoryData ? $oneCategoryData : '';
        }

        if ($_GET['action'] == 'deleteCategory') {

            $categoryData = $categoryObj->deleteCategory($Req);
            $Data     = $categoryData ? $categoryData : '';
        }
