...
if (isset($token) && isset($verify) && $verify) {
    ...
    if ($_GET['action'] == 'getSomeServices') {
        ...
        $categoryData = $ServiceObj1->getSomeServices($start, $aItemsPerPage, $sort, $type, $searchName,$category_type);
    ...
    if ($_GET['action'] == 'getSearchServicesCount') {
        ...
        $categorySearchData = $ServiceObj1->getSearchServicesCount($sort, $type, $searchName,$category_type);
    ...
    if ($_GET['action'] == 'getServicesCount') {
        ...
        $categoryData = $ServiceObj1->getServicesCount($category_type);
        $Data      = $categoryData ? $categoryData : '';
    }

    if ($_GET['action'] == 'addEditService') {
        ...
        $uploadFolder = $Config['uploads_path'] . 'services';
        ...
        $categoryData = $ServiceObj1->addEditService($_REQUEST, $temp['file'], $category_id); 
        $Data = $categoryData?$categoryData:''; 
    }
...
if (isset($postdata) && !empty($postdata)) {
    ...
    if (isset($token) && isset($verify) && $verify) {
        ...
        if ($_GET['action'] == 'getOneCategory') {
            ...
            $oneCategoryData = $ServiceObj1->getOneCategory($Req);
 ...
 if ($_GET['action'] == 'getCategoryByParentID') {
            ...
            $oneCategoryData = $ServiceObj1->getCategoryByParentID($Req,$category_type);
            $Data        = $oneCategoryData ? $oneCategoryData : '';
        }

        if ($_GET['action'] == 'deleteService') {

            $categoryData = $ServiceObj1->deleteService($Req);
            $Data     = $categoryData ? $categoryData : '';
        }
