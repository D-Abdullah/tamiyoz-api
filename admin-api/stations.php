...
//         die();
        ...
        $categoryData = $stationObj->getSomeStations($pagename,$start, $aItemsPerPage, $sort, $type, $searchName);
    ...
    if ($_GET['action'] == 'getSearchStationsCount') {
        ...
        $categorySearchData = $stationObj->getSearchStationsCount($sort, $type, $searchName,$category_type);
    ...
    if ($_GET['action'] == 'getStationsCount') {
        ...
        $categoryData = $stationObj->getStationsCount($category_type);
        $Data      = $categoryData ? $categoryData : '';
    }

    if ($_GET['action'] == 'addEditStation') {
        ...
        $uploadFolder = $Config['uploads_path'] . 'stations';
        ...
        $categoryData = $stationObj->addEditStation($_REQUEST, $temp['file']);
        $Data = $categoryData?$categoryData:''; 
    }
...
if (isset($postdata) && !empty($postdata)) {
    ...
    if (isset($token) && isset($verify) && $verify) {
        ...
        if ($_GET['action'] == 'getOneStation') {
            ...
            $oneCategoryData = $stationObj->getOneStation($Req);
 ...
 if ($_GET['action'] == 'getCategoryByParentID') {
            ...
            $oneCategoryData = $stationObj->getCategoryByParentID($Req,$category_type);
            $Data        = $oneCategoryData ? $oneCategoryData : '';
        }

        if ($_GET['action'] == 'deleteStation') {

            $categoryData = $stationObj->deleteProject($Req);
            $Data     = $categoryData ? $categoryData : '';
        }
