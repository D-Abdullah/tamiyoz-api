...
if (isset($token) && isset($verify) && $verify) {
    ...
    if ($_GET['action'] == 'getSomeTrainingCourses') {
        ...
        $categoryData = $NewsWebObj->getSomeTrainingCourses($start, $aItemsPerPage, $sort, $type, $searchName);
    ...
    if ($_GET['action'] == 'getSearchtrainingCoursesCount') {
        ...
        $categorySearchData = $NewsWebObj->getSearchtrainingCoursesCount($sort, $type, $searchName);
    ...
    if ($_GET['action'] == 'getTrainingCoursesCount') {
        ...
        $categoryData = $NewsWebObj->getTrainingCoursesCount();
        $Data      = $categoryData ? $categoryData : '';
    }
    if ($_GET['action'] == 'getTrainingCategories') {

        $categoryData = $NewsWebObj->getTrainingCategories();
        $Data      = $categoryData ? $categoryData : '';
    }

    if ($_GET['action'] == 'addEditNews') {
        ...
        $uploadFolder = $Config['uploads_path'] . 'news';
        ...
        $categoryData = $NewsWebObj->addEditNews($_REQUEST, $temp['file']);
        $Data = $categoryData?$categoryData:'';
    }
...
if (isset($postdata) && !empty($postdata)) {
    ...
    if (isset($token) && isset($verify) && $verify) {
        ...
        if ($_GET['action'] == 'getOneTrainingCourses') {
            ...
            $oneCategoryData = $NewsWebObj->getOneTrainingCourses($Req);
            $Data        = $oneCategoryData ? $oneCategoryData : '';
        }
        if ($_GET['action'] == 'getCategoryByParentID') {

            $oneCategoryData = $NewsWebObj->getCategoryByParentID($Req);
            $Data        = $oneCategoryData ? $oneCategoryData : '';
        }

        if ($_GET['action'] == 'deleteTrainingCourses') {

            $categoryData = $NewsWebObj->deleteTrainingCourses($Req);
            $Data     = $categoryData ? $categoryData : '';
        }
